...
def ranking():
  ...
  for i in range(len(response['results'])):
    if response['results'][i]['vote_average'] >= 8 :
      movie8.append(response['results'][i])

  for i in range(len(response['results'])):
    for j in range(len(response['results'])-1-i):
      if response['results'][j]['vote_average'] > response['results'][j+1]['vote_average']:
        response['results'][j] , response['results'][j+1] =  response['results'][j+1] , response['results'][j]
  top5 = []
  for i in range(5):
    top5.append(response['results'][len(response['results'])-i-1])
  print(top5)
