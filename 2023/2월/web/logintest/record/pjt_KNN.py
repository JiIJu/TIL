...
def squat_ver1():
    move = pd.read_excel(
        'C:\\Users\\multicampus\\Desktop\\gogo\\S08P12C102\\web\\logintest\\record\\Back_test3_Ver3.xlsx')

    cell = move.iloc[0:802, 0:12]
    label = move.iloc[0:802, 12]

    # 들어오는 데이터 구조.

    ## new_cell = list구조
    # newcell = vid.VideoCamera()
    # newcell.update()
    # new_cell = newcell.oridata[1]
    # test_cell = pd.DataFrame(new_cell)
    ##test_cell = move.iloc[802:, 0:12]
    # print(1)
