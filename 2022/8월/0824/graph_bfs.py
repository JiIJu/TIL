...
def bfs(g,v,n): # g: 그래프(인접리스트) , v:시작지점 , n : 정점의 개수
    # 중복탐색 방지를 위한 방문 배열
    visited = [False] * (n+1) #인덱스가 몇부터 시작인지를 잘 살펴봐야 한다.
    # 정점번호가 1부터 시작하면 n+1
    # 정점번호가 0부터 시작하면 n
    ...
    # 반복을 계속 하는데
    # 초기에 탐색시작 위치를 넣어주고 반복을 진행하기 때문에
    ...
    while queue:
        ...
        for i in g[t]: # 인접 리스트인 g(그래프)에서 t번 정점과 연결된 정점을 모두 탐색한다.
            if not visited[i]: # 다음 정점i를 방문한적이 없다. => 탐색대상이 되고 탐색대상은 큐에 추가
                # 큐에 추가 한 후에 탐색했다 라는 표시를 하기 위해서 방문배열에 체크
                queue.append(i)
                visited[i] = True


graph = [[],[2,3],[1,4,5],[1,7],[2,6],[2,6],[4,7],[3,6]]
bfs(graph,1,7)