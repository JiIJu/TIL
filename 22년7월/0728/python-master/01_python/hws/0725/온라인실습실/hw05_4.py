def fn_d(n):  # n 은 입력으로 들어오는 숫자인데 , 결과로 각 자릿수를 더하고, 자기 자신을 더해서 반환
    # 91 = 9 + 1 + 91 = 101
    # n을 문자열로 바꿔서 쓰는 방법
    number = str(n)
    # 문자열로 바꾸면 각 인덱스 위치에 있는 철자가 바로 자리수가 된다.
    # 앞에서부터 하나씩 떼와서 숫자로 다시 바꾼 다음 더해주면 된다.
    # number 91234
    # i => "9" , "1" , "2" , "3", "4"
...
self_set = number_set - gen_set  # 차집합을 이용해서 빼주면 만들수 없는 숫자들만 남게 된다.


def is_selfnumber(n):  # n 이라는 수기 selfnumber인가??
